...
giniLeftRight trenovaciDataList = 1.0 - sum pravdepodobnosti_na2
  where pocty_trid = map length $ group $ sort $ map trida trenovaciDataList -- pocet jednotlivych trid [A,C,B,A,C,C] -> [A,A,B,C,C,C] -> [AA,B,CCC]-> [2,1,3]
        pravdepodobnosti_na2 = map (\x -> (fromIntegral x / fromIntegral (length trenovaciDataList))*(fromIntegral x / fromIntegral (length trenovaciDataList))) pocty_trid --(pocty_trid/celkem_trid)^2 -> pravdepodobnost vyskytu dane tridy
